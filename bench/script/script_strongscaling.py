...
try:
    os.stat('strongscaling_data_nodes='+str(nodes)+'PPN='+str(CTF_PPN)+'THREADS='+str(OMP_NUM_THREADS))
except:
    os.mkdir('strongscaling_data_nodes='+str(nodes)+'PPN='+str(CTF_PPN)+'THREADS='+str(OMP_NUM_THREADS))    
os.chdir('strongscaling_data_nodes='+str(nodes)+'PPN='+str(CTF_PPN)+'THREADS='+str(OMP_NUM_THREADS))
...
text_file.write("ibrun %s -model CP -tensor r -dim 6 -size 47 -rank 5 -maxiter 5 -filename CP_r_nodes=%s_dim=6_size=47_rank=5_bench.csv -resprint 1\n" % (executive,nodes))
text_file.write("ibrun %s -model Tucker -tensor r2 -dim 6 -size 47 -rank 4 -maxiter 5 -filename Tucker_r2_nodes=%s_dim=6_size=47_rank=4_bench.csv -resprint 1\n" % (executive,nodes))
