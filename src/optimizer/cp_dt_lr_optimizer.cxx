...
CPDTLROptimizer<dtype>::CPDTLROptimizer(int order, int r, int update_rank,
...
template <typename dtype> CPDTLROptimizer<dtype>::~CPDTLROptimizer() {
  // delete S;
...
void CPDTLROptimizer<dtype>::mttkrp_map_init(int left_index) {
  ...
  this->mttkrp_map[seq_tree_top] =
      Tensor<dtype>(strlen(seq_map_init), lens, *dw);
  if (this->low_rank_decomp && count_subiteration > 1) {
    ...
    this->mttkrp_map[seq_tree_top][seq_map_init] =
...
void CPDTLROptimizer<dtype>::mttkrp_map_DT(string index) {
  ...
  this->mttkrp_map[index][index_char] =
      this->mttkrp_map[parent_index][parent_index] *
      this->W[this->indexes[W_index]][mat_index];
}
...
void CPDTLROptimizer<dtype>::update_cached_tensor(int left_index) {
  ...
  if (this->first_subtree) {
    (*cached_tensor1)[seq_map_init] +=
        (*this->V)[seq_V] * this->U[seq_U] * this->VT[seq_VT];
  } else {
    (*cached_tensor2)[seq_map_init] +=
        (*this->V)[seq_V] * this->U[seq_U] * this->VT[seq_VT];
  }
}

template <typename dtype> double CPDTLROptimizer<dtype>::step() {
  ...
  for (int i = 0; i < this->indexes.size(); i++) {
    ...
    this->grad_W[this->indexes[i]]["ij"] =
        -M["ij"] + this->W[this->indexes[i]]["ik"] * this->S["kj"];
    if (((this->first_subtree && i == this->indexes.size() - 1) ||
         (!this->first_subtree && i == 0)) &&
        count_subiteration >= 1) {
      get_rankR_update_cholesky(this->rank, this->U, this->s, this->VT, M,
                                this->W[this->indexes[i]], this->S,
                                this->randomsvd);
      this->W[this->indexes[i]]["ij"] +=
          this->U["ik"] * this->s["k"] * this->VT["kj"];
