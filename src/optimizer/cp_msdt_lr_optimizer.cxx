...
CPMSDTLROptimizer<dtype>::CPMSDTLROptimizer(int order, int r, int update_rank,
...
template <typename dtype> CPMSDTLROptimizer<dtype>::~CPMSDTLROptimizer() {
  // delete S;
  delete[] is_cached;
...
void CPMSDTLROptimizer<dtype>::update_cached_tensor(int left_index) {
  ...
  // cached_tensors[left_index][seq_map_init] = this->W[left_index][seq_matrix]
  // * (*this->V)[seq_V];
  char seq_temp[order];
  int i = 0;
  while (seq_V[i] != '\0') {
  ...
  cached_tensors[left_index][seq_map_init] =
      cached_tensors[left_index][seq_map_init] +
      this->VT[seq_VT] * temp[seq_temp];

  old_W[left_index] = this->W[left_index];
  this->is_cached[left_index] = true;
}

template <typename dtype> double CPMSDTLROptimizer<dtype>::step() {
  ...
  for (int i = 0; i < this->indexes.size(); i++) {
    ...
    if (!is_cached[this->indexes[i]] || (i != (this->indexes.size() - 1))) {
      ...
      get_rankR_update_cholesky(this->rank, this->U, this->s, this->VT, M,
                                this->old_W[this->indexes[i]], this->S,
                                this->randomsvd);
      this->W[this->indexes[i]]["ij"] =
          this->old_W[this->indexes[i]]["ij"] +
