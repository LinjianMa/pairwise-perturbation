...
CPD<dtype>::CPD(int order, int size_, int r, World & dw)
...
template<typename dtype>  
void CPD<dtype>::Init(Tensor<dtype>* input, Matrix<dtype>* mat){
...
template<typename dtype>  
void CPD<dtype>::print_grad(int i) const {
	assert(grad_W != NULL);
	grad_W[i].print();
}
...
template<typename dtype>
void CPD<dtype>::update_gradnorm() {
	gradnorm = 0;
	for (int i=0; i<this->order; i++) { 
...
template<typename dtype>  
bool CPD<dtype>::als(double tol, double timelimit, int maxiter) {

	World * dw = this->world;
	int order = this->order;
	int rank = this->rank[0];
	...
	*/
	Matrix<dtype> S = Matrix<>(rank,rank);
	...
	for (iter=0; iter<=maxiter; iter++)
	{
		...
		if (iter%100==0 || iter==maxiter) {
			//get the gradient
			gradient_CP(*(this->V), this->W, this->grad_W, *dw);
			update_gradnorm();
			if(dw->rank==0) cout << "  [dim]=  " << this->size[0] << "  [iter]=  " << iter << "  [gradnorm]  "<< gradnorm << "  [tol]  " << tol  <<  "\n";
		...
		for (int i=0; i<order; i++) { 
			...
			*/
			...
			// calculating S
			S["ij"] = this->W[index[0]]["ki"] * this->W[index[0]]["kj"];
			for (int ii=1; ii<order-1; ii++) {
				S["ij"] = S["ij"]*(this->W[index[ii]]["ki"] * this->W[index[ii]]["kj"]);
			}
			// subproblem M=W*S
			SVD_solve(M, this->W[i], S);
			// recover the char
			swap_char(seq_V, i, order-1);
		}
		Normalize(this->W, order, *dw);
		// print .
 ...
 */
template<typename dtype>  
bool CPD<dtype>::alsCP_DT(double tol, 
						  double timelimit, 
						  int maxiter, 
						  double lambda,
        				  ofstream & Plot_File,
    ...
    Matrix<> regul = Matrix<>(this->W[0].ncol,this->W[0].ncol);
    regul["ii"] =  1.*lambda;
