...
def fetch_all_traces():
    ...
    """
    all_traces = {'grad_iter_trace': [], 'residual_iter_trace': [], 'grad_time_trace': [], 'residual_time_trace': []}
    # for all files
...
def generate_trace_from_csv(filename):
    ...
    grad_iter_trace = dict(
        x=df['[iter]'].tolist(),
        y=df['[gradnorm]'].tolist(),
        mode="markers+lines",
        type='custom',
        marker={'color': color,
                'symbol': 104,
    ...
    residual_time_trace = dict(
        ...
        name=model_prefix)
    return {
        'grad_iter_trace': grad_iter_trace,
        'residual_iter_trace': residual_iter_trace,
        'grad_time_trace': grad_time_trace,
...
while True:
    ...
    vis._send({
        'data': all_traces['grad_iter_trace'],
        'layout': grad_iter_layout,
        'win': 'win1'
    })
    vis._send({
    ...
    vis._send({
        'data': all_traces['grad_time_trace'],
        'layout': grad_time_layout,
        'win': 'win3'
    })
    vis._send({
