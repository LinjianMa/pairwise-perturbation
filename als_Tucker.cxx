...
Tensor<> get_core_tensor(Tensor<> &T, Matrix<> *factor_matrices, int ranks[],
  ...
  for (int i = 0; i < T.order; i++) {
    core_arg[i] = 'a';
    matrix_arg[1] = arg[i];
    Matrix<double> transpose(factor_matrices[i].ncol, factor_matrices[i].nrow,
                             dw);
    transpose["ij"] = factor_matrices[i]["ji"];
    core_tensors[i + 1][core_arg] =
        transpose[matrix_arg] * core_tensors[i][arg];
    core_arg[i] = arg[i];
  }
...
void ttmc_map_DT(map<string, Tensor<>> &ttmc_map, map<string, string> &parent,
  ...
  if (ttmc_map.find(args) != ttmc_map.end())
    return;
  char seq_w[3];
  seq_w[2] = '\0';
  seq_w[1] = '*';
      ...
      if (bench == false) {
        if (dw.rank == 0) {
          cout << "  [dim]=  " << V.lens[0] << "  [iter]=  " << iter
               ...
               << "  [dtime]  " << dtime << "\n";
          // plot to file
          Plot_File << V.lens[0] << "," << iter << "," << diffnorm << "," << tol
                    << "," << 0 << "," << diffnorm_V << "," << dtime << "\n";
    ...
    for (int i = 0; i < V.order; i++) {
      ...
      if (sibling[args].length() == 1) {
        ...
        if (args[0] == parent[args][0])
          seq_A[0] = parent[args][1];
        else
          seq_A[0] = parent[args][0];
        seq_Y[int(seq_A[0] - 'a')] = '*';
        ...
        Y[seq_Y] = ttmc_map[parent[args]][seq] *
...
void Build_ttmc_map(map<string, Tensor<>> &ttmc_map, Tensor<> &V, Matrix<> *W,
  ...
  ttmc_map[args][seq1_contract] =
      M[seq2_contract] * W[int(args3[0] - 'a')][args3];
}
...
void alsTucker_DT_sub(Tensor<> &V, Tensor<> &core, Tensor<> &core_prev,
  ...
  for (; iter <= maxiter; iter++) {
    ...
    for (int i = 0; i < V.order; i++) {
      ...
      S["i"] = check["ii"];
      Transform<double, double>([](double &b) {
        if (b > 0)
          b = 1;
        else
          b = -1;
...
bool alsTucker_PP(Tensor<> &V, Tensor<> &core, Matrix<> *W, double tol,
  ...
  while (diffnorm > tol && iter <= maxiter) {
    ...
    if (bench == false) {
      ...
      alsTucker_DT_sub(V, core, core_prev, W, dW, tol, tol_init, timelimit,
                       maxiter, st_time, Plot_File, diffnorm, iter, resprint,
                       dw);
    }
    ...
    alsTucker_PP_sub(V, core, core_prev, W, dW, tol, tol_init, timelimit,
                     maxiter, st_time, Plot_File, diffnorm, iter, resprint,
                     bench, dw);
